declare module 'lodash/fp' {
  ...
  type MapWithPropertyIterateeShorthand = {
    <T, K extends keyof T>(propertyIteratee: K, collection: Collection<
      T
    >): Array<T[K]>;
  ...
  var uncappedMap: {
    ...
    <T, U extends Dictionary<T>, R>(iteratee: (
      item: T,
      key: string,
      object: U,
    ) => R, object: U): Array<R>;
  ...
  var filter: {
    ...
    <T, U extends Dictionary<T>, K extends keyof U>(matchesIteratee:
      | [K, U[K]]
      | Partial<U>, collection: Collection<U>): Array<U>;
    <T, U extends Dictionary<T>, K extends keyof U>(matchesIteratee:
      | [K, U[K]]
      | Partial<U>): (collection: Collection<U>) => Array<U>;
  };
  ...
  var compose: {
    <A, B, C, D, E, F, R>(fr: (f: F) => R, ef: (e: E) => F, de: (
      d: D,
    ) => E, cd: (c: C) => D, bc: (b: B) => C, ab: (a: A) => B): (a: A) => R;
    <A, B, C, D, E, R>(er: (e: E) => R, de: (d: D) => E, cd: (c: C) => D, bc: (
      b: B,
    ) => C, ab: (a: A) => B): (a: A) => R;
    <A, B, C, D, R>(dr: (d: D) => R, cd: (c: C) => D, bc: (b: B) => C, ab: (
      a: A,
    ) => B): (a: A) => R;
